...
>Note: For the great majority of users, using AWS Batch is best done by using a workflow manager such as [WDL workflows](/datascience/wdl_workflows/) (executed via [Cromwell or other engines](/datademos/wdl_execution_engines/)) or [Nextflow](/compdemos/nextflow/), both of which are supported at the Hutch and can allow you to leverage cloud computing resources such as AWS Batch without creating a workflow that will ONLY work on AWS Batch.  
