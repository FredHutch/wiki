...
A note about screen:  `ssh rhino` will assign you to one of the available, specific servers (ie: rhino1, rhino2, rhino3) and _screen_ instances _are not shared across the servers_. If you are interested in using screen to run a persistent session on the rhino cluster, consider:
 1. accessing a specific server directly eg: `ssh rhino1 ` 
