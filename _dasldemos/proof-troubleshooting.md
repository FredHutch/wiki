...
- By default, PROOF servers run for seven days before they shut down automatically. If submitting/validating a workflow produces an error message like “Your server is not up!”, you’ll need to start up another PROOF server via [the “PROOF Server” tab](/dasldemos/proof-how-to#setup-your-proof-server). Analysis jobs in progress will not be affected by the restart and all previous job history will be saved.
...
- If the screen is grayed out and you’re unable to press buttons at all, your browser has timed out and just needs a quick refresh.
...
- If your server has been running for a few days, it is definitely worth [stopping and restarting](/dasldemos/proof-how-to/#setup-your-proof-server) your PROOF server. If any recent updates have been pushed, your new server will pick them up and hopefully that will fix your issue. And even if nothing's been updated, always worth trying the old standby of "turning it off and on again."

## Workflow Level Issues

### Validate your workflow

- Before submitting a job, validate your WDL script and an input json via [the “Validate” tab in PROOF](/dasldemos/proof-how-to#validate-a-wdl-workflow). This will catch typos and larger structural issues in your script. Ideally, you should see something to the effect of `$valid [1] TRUE` , but here are a few of the non-ideal scenarios:
...
- Start by navigating to [the “Track Jobs” tab of PROOF](/dasldemos/proof-how-to#track-wdl-workflow-runs) and identify the workflow run of interest from a list of recent jobs run via PROOF.
- Click on the "Workflow Details" button for the entry corresponding to the workflow run in question. This will automatically bring you to the "Workflow Details" tab of PROOF populated with details for that run.
- By default, you will start in the "Job List" subtab which you can scroll through to identify tasks with an "executionStatus" of "Failed". This will narrow down your issue to a particular task and the table itself will provide information that will be useful in your troubleshooting endeavors.

![job_list_failed](/dasldemos/assets/proof_ts_job_list_failed_task.png)

### Identify why the task failed

- Near the top of the "Workflow Details" tab, you'll notice a subtab called "Job Failures". Navigate to that subtab and click “Get/Refresh Failed Job Metadata” to populate the table with details about your failed task. The `failures.causedBy.message` columns are of particular interest as they contain the error messages produced by the task. In this example pictured below, I provided an erroneous path for one of my inputs, and as you can see, the tool “could not localize” it:

![job_failures](/dasldemos/assets/proof_ts_job_failures.png)

- If these messages don’t provide enough information, navigate back to the “Job List” subtab and take a look at a few additional columns:
...
- If these resources still don’t provide any sort of illumination, navigate back to the "Workflow Details" of the workflow run of interest and click on the ["Troubleshoot" subtab](](/dasldemos/proof-how-to#get-workflow-run-details)). This should display a large unformatted json, mostly containing the same information as the tables mentioned above, but it could contain a detail that was missed by PROOF’s user interface.
- Along the same lines, you can also look into the raw workflow log produced by your underlying PROOF server. To do this, identify your workflow log directory on the "Server” tab (see screenshot below) and navigate to that directory on Rhino. Once there, you will see a collection of files following the naming convention of `workflow.[workflowid].log`. Open the logfile corresponding to the workflow in question using your text viewer of choice (`more`, `less`, `cat`, VS Code) to see details about the Cromwell execution of your WDL script.
...
- If you’re creating/troubleshooting your own Docker image and you push an update to the same version tag, you may need to clear your [Apptainer cache](https://apptainer.org/docs/user/main/cli/apptainer_cache_clean.html) for PROOF to download a new copy of the image. Otherwise, PROOF will just reuse the previous cached version of the image, therefore not incorporating your changes.
- To remove the cached images, start by identifying your Apptainer cache directory at the bottom of the ["Server” tab](/dasldemos/proof-how-to#setup-your-proof-server) of PROOF (see screenshot below). Navigate to that directory in Rhino and delete all files in that directory ending in `.sif`.
