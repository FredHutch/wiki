...
1. Load lmod: `source /app/lmod/lmod/init/profile`
2. Load in `apptainer` using `module load Apptainer/1.1.6` 
3. Use `apptainer` to `pull` a container (note that we recommend using Docker containers). For example, `apptainer pull biocontainers/samtools:v1.9-4-deb_cv1`
4. Use Apptainer to`run` a container on your data. For example: `apptainer run biocontainers/samtools samtools view -c my_bam_file.bam > counts.txt`  will run `samtools view` and count the number of reads in your file.
...
Use Nextflow to run your software. Requires knowledge of Nextflow and Nextflow workflows.
...
2. I recommend installing `miniconda` using the shell script (there are other options to install conda software, including `mamba`). Make sure the `miniconda` directory is added to your `$PATH` variable so you can use the `conda` command. For example, I would alter my `.bashrc` file to contain the line: `export PATH=$PATH:/home/tladera2/miniconda3/bin/` 
...
Note that some channels require a subscription, but `bioconda` and `conda-forge` do not.
...
1. When using your software, activate your environment using `conda activate` . For example: `conda activate samtools_env` 
...
2. Ask SciComp if it’s possible to make it into a module (they know a lot!)
3. Before doing anything: look on Biostars or Stack Overflow to see if there’s compilation advice.
4. Compile following the instructions. Be prepared to debug a number of steps.
5. Add your compiled binary to your `$PATH` in your `.bashrc` file
...
Multiple process example - module load, 4 commands - break out into multiple pieces

Transitioning to WDL scripts from a multiprocess script. Here’s an example where we are running `multiqc` and `bwa mem` on a fasta file.
