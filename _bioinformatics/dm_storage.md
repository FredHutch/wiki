...
- Databases
- File-Based Storage, such as fh/fast or the "Z drive" often mapped directly to workstations)
...
Currently it is recommended to use a combination of *Economy File*, *Scratch* and *Fast File*.
...
\*\* All admin contact can be initiated by emailing `helpdesk`, but different system administrators are primary for different platforms.  Admin assistance can be requested for data transfers and validation, data import, as well as restoring data from backups for a given resource if available.
